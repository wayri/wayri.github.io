document.getElementById('year').textContent = new Date().getFullYear();
        ...
        function toggleTheme() {
            ...
            if (currentTheme === 'dark-gold') {
                ...
                localStorage.setItem('theme', 'light-hasl');
                label.textContent = "Light-hASL";
            } else {
                body.setAttribute('data-theme', 'dark-gold');
                localStorage.setItem('theme', 'dark-gold');
                label.textContent = "Dark-Gold";
            }
            setTimeout(() => { 
        ...
        if (savedTheme === 'light-hasl') {
            document.documentElement.removeAttribute('data-theme');
            document.getElementById('theme-label').textContent = "Light-hASL";
        } else {
            document.documentElement.setAttribute('data-theme', 'dark-gold');
            document.getElementById('theme-label').textContent = "Dark-Gold";
        }
        ...
        function navigate(pageId) {
            ...
            document.querySelectorAll('.nav-linkΩ').forEach(el => {
                if(el.dataset.target === pageId) {
                    el.classList.add('text-themeAccent', 'underline', 'decoration-2', 'underline-offset-4');
                } else {
                    el.classList.remove('text-themeAccent', 'underline', 'decoration-2', 'underline-offset-4');
                }
            });
            window.scrollTo(0, 0);
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if(hash === 'tools') navigate('tools');
            else if(hash === 'products') navigate('products');
        ...
        // E-SERIES & SUGGESTION LOGIc
        const E12 = [1.0, 1.2, 1.5, 1.8, 2.2, 2.7, 3.3, 3.9, 4.7, 5.6, 6.8, 8.2];
        const E24 = [1.0, 1.1, 1.2, 1.3, 1.5, 1.6, 1.8, 2.0, 2.2, 2.4, 2.7, 3.0, 3.3, 3.6, 3.9, 4.3, 4.7, 5.1, 5.6, 6.2, 6.8, 7.5, 8.2, 9.1];
        
        function getNearest(val, series) {
            if(!val || val <= 0) return { val: 0, err: 0 };
            let mag = Math.pow(10, Math.floor(Math.log10(val)));
            let norm = val / mag;
            if(norm < 1.0) { mag /= 10; norm *= 10; }
            
            let closest = series[0];
            let minDiff = Math.abs(norm - closest);
            for(let i=1; i<series.length; i++){
                let diff = Math.abs(norm - series[i]);
                if(diff < minDiff){ minDiff = diff; closest = series[i]; }
            }
            if(Math.abs(norm - 10) < minDiff) { closest = 1.0; mag *= 10; }
            const res = closest * mag;
            return { val: res, err: ((res - val)/val)*100 };
        }

        function getNearestE96(val) {
            if(!val || val <= 0) return { val: 0, err: 0 };
            let mag = Math.pow(10, Math.floor(Math.log10(val)));
            let norm = val / mag;
            if(norm < 1.0) { mag /= 10; norm *= 10; }

            let i = Math.round(96 * Math.log10(norm));
            let resNorm = parseFloat(Math.pow(10, i/96).toFixed(2));
            if(resNorm >= 10.0) { resNorm = 1.0; mag *= 10; }
            const res = resNorm * mag;
            return { val: res, err: ((res - val)/val)*100 };
        }
        ...
        function applyVal(id, val) {
            if(val) document.getElementById(id).value = parseFloat(val).toFixed(0);
        }

        function updateSuggestion(id, val) {
            let sug = document.getElementById('sug-' + id);
            if(val > 0 && sug) {
        ...
        function runESeries() {
            const valEl = document.getElementById('eseries-in');
            if (!valEl) return;
            const val = parseFloat(valEl.value);
            if(!val) return;
            const e12 = getNearest(val, E12);
        ...
        // SMART VOLTAGE DIVIDER LOGIc
        function setupDividerMode() {
            const divInputs = {
                vin: document.getElementById('calc-vin'),
                r1: document.getElementById('calc-r1'),
                r2: document.getElementById('calc-r2'),
                vout: document.getElementById('calc-vout')
            };

            const targetEl = document.querySelector('input[name="solve_target"]:checked');
            if(!targetEl) return;
            const target = targetEl.value;
            
            for (let key in divInputs) {
                if(!divInputs[key]) continue;
                divInputs[key].readOnly = false;
                divInputs[key].classList.remove('border-2', 'border-themeAccent', 'bg-opacity-10', 'bg-themeAccent', 'font-bold', 'text-themeAccent');
                
                if(key === target) {
                    divInputs[key].readOnly = true;
                    divInputs[key].classList.add('border-2', 'border-themeAccent', 'bg-opacity-10', 'bg-themeAccent', 'font-bold', 'text-themeAccent');
                }
            }
            runSmartDivider();
        }

        function getPackageRecommendation(powerW) {
            if (powerW <= 0.0625) return "0402 (1/16W)";
            if (powerW <= 0.1) return "0603 (1/10W)";
            if (powerW <= 0.125) return "0805 (1/8W)";
            if (powerW <= 0.25) return "1206 (1/4W)";
            if (powerW <= 0.5) return "2010 (1/2W)";
            if (powerW <= 1.0) return "2512 (1W)";
            return `Power Res (${Math.ceil(powerW*1.5)}W+)`;
        }

        function runSmartDivider() {
            const divInputs = {
                vin: document.getElementById('calc-vin'),
                r1: document.getElementById('calc-r1'),
                r2: document.getElementById('calc-r2'),
            ...
            let vals = {
                vin: parseFloat(divInputs.vin?.value),
                r1: parseFloat(divInputs.r1?.value),
                r2: parseFloat(divInputs.r2?.value),
                vout: parseFloat(divInputs.vout?.value)
            };
            ...
            try {
                ...
                if (vals.vin && vals.r1 > 0 && vals.r2 > 0) {
                    ...
                    const pR1 = (current * current) * vals.r1;
                    const pR2 = (current * current) * vals.r2;

                    const pR1El = document.getElementById('pow-r1');
                    const pR2El = document.getElementById('pow-r2');
                    const pkgR1El = document.getElementById('pkg-r1');
                    const pkgR2El = document.getElementById('pkg-r2');
                    
                    if(pR1El) pR1El.textContent = (pR1 * 1000).toFixed(2) + " mW";
                    if(pR2El) pR2El.textContent = (pR2 * 1000).toFixed(2) + " mW";
                    if(pkgR1El) pkgR1El.textContent = getPackageRecommendation(pR1);
                    if(pkgR2El) pkgR2El.textContent = getPackageRecommendation(pR2);
                }
            } catch (e) {}
        }

        // IPc-2221 TRAcE WIDTh LOGIc
        function runPcBTrace() {
            const iEl = document.getElementById('pcb-i');
            const dtEl = document.getElementById('pcb-dt');
            const thEl = document.getElementById('pcb-th');
            
            if(!iEl || !dtEl || !thEl) return;
            ...
            const widthMils = area / (th * 1.378);
            const widthMm = widthMils * 0.0254;
            
            const milEl = document.getElementById('pcb-w-mil');
            const mmEl = document.getElementById('pcb-w-mm');
            
            if(milEl) milEl.textContent = widthMils.toFixed(1) + " mil";
            if(mmEl) mmEl.textContent = widthMm.toFixed(3) + " mm";
        }

        // cOMPENSATOR (TYPE II / III)
        function runcompensator() {
            const typeEl = document.getElementById('comp-type');
            if(!typeEl) return;
            const type = typeEl.value;
            const r1 = parseFloat(document.getElementById('comp-r1').value) * 1e3;
            const r2 = parseFloat(document.getElementById('comp-r2').value) * 1e3;
            const r3 = parseFloat(document.getElementById('comp-r3').value) * 1e3;
            const c1 = parseFloat(document.getElementById('comp-c1').value) * 1e-9;
            const c2 = parseFloat(document.getElementById('comp-c2').value) * 1e-9;
            const c3 = parseFloat(document.getElementById('comp-c3').value) * 1e-9;

            const r3Grp = document.getElementById('comp-r3-grp');
            const c3Grp = document.getElementById('comp-c3-grp');
            const fz2Grp = document.getElementById('comp-fz2-grp');
            const fp2Grp = document.getElementById('comp-fp2-grp');
            const r3El = document.getElementById('comp-r3');
            const c3El = document.getElementById('comp-c3');

            if (type === 'type2') {
                r3Grp.classList.add('opacity-50'); c3Grp.classList.add('opacity-50');
                fz2Grp.classList.add('opacity-50'); fp2Grp.classList.add('opacity-50');
                r3El.disabled = true; c3El.disabled = true;
                
                if(!r2 || !c2 || !c1) return;

                const fz1 = 1 / (2 * Math.PI * r2 * c2);
                const fp1 = 1 / (2 * Math.PI * r2 * ((c1 * c2) / (c1 + c2)));

                document.getElementById('comp-fz1').textContent = formatFreq(fz1);
                ...
                r3El.disabled = false; c3El.disabled = false;

                if(!r1 || !r2 || !r3 || !c1 || !c2 || !c3) return;

                const fz1 = 1 / (2 * Math.PI * r2 * c2);
                const fp1 = 1 / (2 * Math.PI * r2 * ((c1 * c2) / (c1 + c2)));
                const fz2 = 1 / (2 * Math.PI * (r1 + r3) * c3);
                const fp2 = 1 / (2 * Math.PI * r3 * c3);
        ...
        // SMPS / TRANSFORMER
        function runSMPS() {
            const typeEl = document.getElementById('smps-type');
            if(!typeEl) return;
            const type = typeEl.value;
            const vin = parseFloat(document.getElementById('smps-vin').value);
            const vout = parseFloat(document.getElementById('smps-vout').value);
            const ratioGrp = document.getElementById('smps-ratio-grp');

            if(!vin || !vout) return;

            let duty = 0;
            if (type === 'buck') {
                ratioGrp.classList.add('opacity-50');
                document.getElementById('smps-ratio').textContent = "--";
                if (vout > vin) {
                    document.getElementById('smps-duty').textContent = "INVALID";
                    return;
                }
                duty = vout / vin;
            } 
                ...
                const ratio = vor / vout;
                document.getElementById('smps-ratio').textContent = ratio.toFixed(2) + " : 1";
            }

            document.getElementById('smps-duty').textContent = (duty * 100).toFixed(1) + "%";
        }

        // VOLTAGE MARGINING
        function runMargin() {
            const vrefEl = document.getElementById('vm-vref');
            if(!vrefEl) return;
            ...
            if(!vref || !rtop || !rbot || !rinj) return;

            const vnom = vref * (1 + (rtop / rbot));
            document.getElementById('vm-vnom').textContent = vnom.toFixed(2) + " V";

            const term1 = vref * (1/rtop + 1/rbot + 1/rinj);
            const term2 = vdac / rinj;
            const vmarg = (term1 - term2) * rtop;

            document.getElementById('vm-vmarg').textContent = vmarg.toFixed(2) + " V";
        }

        // MOTOR DYNAMIcS
        function runMotor() {
            const pEl = document.getElementById('mot-p');
            if(!pEl) return;
            const p = parseFloat(pEl.value);
            const f = parseFloat(document.getElementById('mot-f').value);
            const t = parseFloat(document.getElementById('mot-t').value);

            if(!p || !f || !t) return;

            const rpm = (120 * f) / p;
            document.getElementById('mot-rpm').textContent = rpm.toFixed(0) + " RPM";

            const powerKw = (t * rpm) / 9550;
            const powerhp = powerKw * 1.34102;

            document.getElementById('mot-kw').textContent = powerKw.toFixed(2);
            document.getElementById('mot-hp').textContent = powerhp.toFixed(2);
        }

        // ThERMISTOR PLOTTER
        function runThermistor() {
            const r25El = document.getElementById('ntc-r25');
            if(!r25El) return;
            const r25 = parseFloat(r25El.value);
            const beta = parseFloat(document.getElementById('ntc-beta').value);
            const tc = parseFloat(document.getElementById('ntc-t').value);
            if(!r25 || !beta) return;
        ...
        function drawThermistorPlot(r25, beta) {
            addGraphCursor('ntc-canvas', (nx, ny) => { const t = -40 + nx * 165; const r = parseFloat(document.getElementById('ntc-r25').value) * Math.pow(10, -1 + ny * 3); return t.toFixed(1)+'\u00b0C, ' + (r>=1000 ? (r/1000).toFixed(2)+'k\u03a9' : Math.round(r)+'\u03a9'); });
            const canvas = document.getElementById('ntc-canvas');
            ...
            const minT = -40, maxT = 125;
            const pts = [];
            let maxLogR = -Infinity, minLogR = Infinity;
            
            for(let t = minT; t <= maxT; t+=5) {
                const r = r25 * Math.exp(beta * (1/(t+273.15) - 1/298.15));
            ...
            ctx.strokeStyle = style.getPropertyValue('--accent-color').trim() || '#D4AF37';
            ctx.lineWidth = 2;
            ctx.beginPath();
            
            pts.forEach((p, i) => {
                const x = ((p.t - minT) / (maxT - minT)) * w;
                const y = h - ((p.logR - minLogR) / (maxLogR - minLogR)) * h;
        ...
        // STEAMPUNK ANOMALOUS cLOcK WIDGET + ANIMATION
        let steamAnimId;
        let clockEl = null;
    ...
    "127": [[-27.1, 83.52], [-20.85, 82.73], [-22.69, 82.34], [-26.52, 82.3], [-31.9, 82.2], [-31.4, 82.02], [-27.86, 82.13], [-24.84, 81.79], [-22.9, 82.09], [-22.07, 81.73], [-23.17, 81.15], [-20.62, 81.52], [-15.77, 81.91], [-12.77, 81.72], [-12.21, 81.29], [-16.29, 80.58], [-16.85, 80.35], [-20.05, 80.18], [-17.73, 80.13], [-18.9, 79.4], [-19.7, 78.75], [-19.67, 77.64], [-18.47, 76.99], [-20.04, 76.94], [-21.68, 76.63], [-19.83, 76.1], [-19.6, 75.25], [-20.67, 75.16], [-19.37, 74.3], [-21.59, 74.22], [-20.43, 73.82], [-20.76, 73.46], [-22.17, 73.31], [-23.57, 73.31], [-22.31, 72.63], [-22.3, 72.18], [-24.28, 72.6], [-24.79, 72.33], [-23.44, 72.08], [-22.13, 71.47], [-21.75, 70.66], [-23.54, 70.47], [-24.31, 70.86], [-25.54, 71.43], [-25.2, 70.75], [-26.36, 70.23], [-23.73, 70.18], [-22.35, 70.13], [-25.03, 69.26], [-27.75, 68.47], [-30.67, 68.13], [-31.78, 68.12], [-32.81, 67.74], [-34.2, 66.68], [-36.35, 65.98], [-37.04, 65.94], [-38.38, 65.69], [-39.81, 65.46], [-40.67, 64.84], [-40.68, 64.14], [-41.19, 63.48], [-42.82, 62.68], [-42.42, 61.9], [-42.87, 61.07], [-43.38, 60.1], [-44.79, 60.04], [-46.26, 60.85], [-48.26, 60.86], [-49.23, 61.41], [-49.9, 62.38], [-51.63, 63.63], [-52.14, 64.28], [-52.28, 65.18], [-53.66, 66.1], [-53.3, 66.84], [-53.97, 67.19], [-52.98, 68.36], [-51.48, 68.73], [-51.08, 69.15], [-50.87, 69.93], [-52.01, 69.57], [-52.56, 69.43], [-53.46, 69.28], [-54.68, 69.61], [-54.75, 70.29], [-54.36, 70.82], [-53.43, 70.84], [-51.39, 70.57], [-53.11, 71.2], [-54.0, 71.55], [-55, 71.41], [-55.83, 71.65], [-54.72, 72.59], [-55.33, 72.96], [-56.12, 73.65], [-57.32, 74.71], [-58.6, 75.1], [-58.59, 75.52], [-61.27, 76.1], [-63.39, 76.18], [-66.06, 76.13], [-68.5, 76.06], [-69.66, 76.38], [-71.4, 77.01], [-68.78, 77.32], [-66.76, 77.38], [-71.04, 77.64], [-73.3, 78.04], [-73.16, 78.43], [-69.37, 78.91], [-65.71, 79.39], [-65.32, 79.76], [-68.02, 80.12], [-67.15, 80.52], [-63.69, 81.21], [-62.23, 81.32], [-62.65, 81.77], [-60.28, 82.03], [-57.21, 82.19], [-54.13, 82.2], [-53.04, 81.89], [-50.39, 82.44], [-48.0, 82.06], [-46.6, 81.99], [-44.52, 81.66], [-46.9, 82.2], [-46.76, 82.63], [-43.41, 83.23], [-39.9, 83.18], [-38.62, 83.55], [-35.09, 83.65], [-27.1, 83.52]],
        ...
        function drawSolarMap() {
            ...
            canvas.width = (canvas.clientWidth || 600) * dpr;
            canvas.height = (canvas.clientHeight || 180) * dpr;
            const ctx = canvas.getContext('2d');
            ctx.scale(dpr, dpr);
            ...
            ctx.fillStyle='rgba(80,100,70,0.7)'; ctx.strokeStyle='rgba(120,150,100,0.5)'; ctx.lineWidth=0.5;
            for(const key in _WMAP) {
                const pts=_WMAP[key]; ctx.beginPath();
                const [x0,y0]=ll(pts[0][0],pts[0][1]); ctx.moveTo(x0,y0);
                for(let i=1;i<pts.length;i++){const[x,y]=ll(pts[i][0],pts[i][1]);ctx.lineTo(x,y);}
                ctx.closePath(); ctx.fill(); ctx.stroke();
            }

            const decl = _solarDecl(_solarMonth);
            const sunY = (90-decl)/180*h;
            ctx.strokeStyle='rgba(255,200,0,0.3)'; ctx.lineWidth=1; ctx.setLineDash([6,4]);
            ctx.beginPath(); ctx.moveTo(0,sunY); ctx.lineTo(w,sunY); ctx.stroke();
            ctx.setLineDash([]);

            if(_solarLat !== null) {
                const [mx,my]=ll(_solarLon,_solarLat);
                ctx.beginPath(); ctx.arc(mx,my,5,0,Math.PI*2);
                ctx.fillStyle='#D4AF37'; ctx.fill();
                ctx.strokeStyle='#fff'; ctx.lineWidth=1.5; ctx.stroke();
                ctx.strokeStyle='rgba(212,175,55,0.35)'; ctx.setLineDash([3,3]);
            ...
            const mn=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            ctx.fillStyle='rgba(212,175,55,0.85)'; ctx.font='10px "JetBrains Mono"';
            ctx.fillText('Month: '+mn[_solarMonth]+'  |  Solar decl: '+decl.toFixed(1)+'°', 8, 14);
        }

        function updateSolarSim() {
            const timeEl=document.getElementById('solar-time');
            const labelEl=document.getElementById('solar-time-label');
            const locEl=document.getElementById('pv-loc');
            const tiltEl=document.getElementById('pv-tilt');
            const irrEl=document.getElementById('pv-irr');
            const pwrEl=document.getElementById('pv-pwr');
            if(!timeEl) return;
            const hour=parseFloat(timeEl.value);
            ...
            const decl=_solarDecl(_solarMonth);
            const tilt=_optTilt(_solarLat,decl);
        ...
        let port, reader;
        let serialBuffer = "";
        const plotData = new Array(100).fill(0);
        let plotctx, plotEl;

        function initPlotter() {
            plotEl = document.getElementById('plotter-canvas');
            if(!plotEl) return;
        ...
        function addGraphCursor(canvasId, valueFormatter) {
            ...
            if (!canvas || canvas.hasAttribute('data-cursor-attached')) return;
            canvas.setAttribute('data-cursor-attached', 'true');
            
            let overlayCanvas = document.createElement('canvas');
            overlayCanvas.id = canvasId + '-overlay';
            overlayCanvas.className = canvas.className;
            ...
            canvas.addEventListener('mousemove', (e) => {
                ...
                const text = valueFormatter(x / overlayCanvas.width, 1 - (y / overlayCanvas.height));
                ctx.font = '10px "JetBrains Mono"';
                const tw = ctx.measureText(text).width;
                let tx = x + 5; let ty = y - 5;
                if(tx + tw > overlayCanvas.width) tx = x - tw - 5;
                if(ty < 10) ty = y + 15;
        ...
        // BODE PLOTTER
        function runBode() {
            const r = parseFloat(document.getElementById('bode-r')?.value) || 1000;
            const cVal = parseFloat(document.getElementById('bode-c')?.value) || 100;
            const type = document.querySelector('input[name="bode-type"]:checked')?.value || 'rc';
            const lbl = document.getElementById('bode-cl');
            let fc;
            if (type === 'rc') {
                if(lbl) lbl.textContent = 'C (nF)';
                fc = 1 / (2 * Math.PI * r * cVal * 1e-9);
            } else {
                if(lbl) lbl.textContent = 'L (\u00b5H)';
                fc = r / (2 * Math.PI * cVal * 1e-6);
            }
            const fcEl = document.getElementById('bode-fc');
            if(fcEl) fcEl.textContent = formatFreq(fc);
            // Draw Bode magnitude
            ...
            for(let i=0;i<=w;i++){
                ...
                const y = h/2 - (dB/40)*h;
                i===0?ctx.moveTo(i,Math.max(0,Math.min(h,y))):ctx.lineTo(i,Math.max(0,Math.min(h,y)));
            }
            ctx.stroke();
        }
        ...
        // BATTERY PAcK
        function runBattery() {
            const v=parseFloat(document.getElementById('bat-v')?.value)||3.7;
            const c=parseFloat(document.getElementById('bat-c')?.value)||3000;
            const s=parseInt(document.getElementById('bat-s')?.value)||4;
            const p=parseInt(document.getElementById('bat-p')?.value)||2;
            const pv=v*s, pc=c*p, pe=(pv*pc/1000);
            const pvEl=document.getElementById('bat-pv');
            const pcEl=document.getElementById('bat-pc');
            const peEl=document.getElementById('bat-pe');
            if(pvEl) pvEl.textContent=pv.toFixed(1)+' V';
            if(pcEl) pcEl.textContent=pc.toFixed(0)+' mAh';
            if(peEl) peEl.textContent=pe.toFixed(1)+' Wh';
        }

        // AWG
        const AWG_DATA = {4:{dia:5.189,res:0.2485,amp:95},6:{dia:4.115,res:0.3951,amp:75},8:{dia:3.264,res:0.6282,amp:55},
            10:{dia:2.588,res:0.9989,amp:30},12:{dia:2.053,res:1.588,amp:20},14:{dia:1.628,res:2.525,amp:15},
            16:{dia:1.291,res:4.016,amp:13},18:{dia:1.024,res:6.385,amp:10},20:{dia:0.812,res:10.15,amp:7.5},
            22:{dia:0.644,res:16.14,amp:5},24:{dia:0.511,res:25.67,amp:3.5}};
        function runAWG(){
            const awg=parseInt(document.getElementById('awg-sel')?.value)||12;
            const d=AWG_DATA[awg]||AWG_DATA[12];
            const diaEl=document.getElementById('awg-dia');
        ...
        // INDUcTOR SIZING
        function runInductor(){
            const mode=document.getElementById('ind-mode')?.value||'buck';
            const vin=parseFloat(document.getElementById('ind-vin')?.value)||12;
            const vout=parseFloat(document.getElementById('ind-vout')?.value)||5;
            const iout=parseFloat(document.getElementById('ind-iout')?.value)||2;
            const fsw=(parseFloat(document.getElementById('ind-fsw')?.value)||200)*1e3;
            const ripplePct=(parseFloat(document.getElementById('ind-ripple')?.value)||30)/100;
            let D, L_uh;
            if(mode==='buck'){D=vout/vin; const dIL=ripplePct*iout; L_uh=(vout*(1-D))/(fsw*dIL)*1e6;}
